...
multi-client race condition, two LangChain dependency/compatibility issues)
that got found and fixed along the way, not just theorized about. Each
section documents the specific before/after.
...
call, not an oversight — see [Notes on design choices](#notes-on-design-choices).
...
the more expensive way). See [Source catalog](#source-catalog) and
[Scope gate](#scope-gate).

## How it works

A question comes in through one interface (CLI or API) and is **routed**
to one of two independent paths:
...
- **RAG over documents**: the question is rewritten in light of conversation
  ...
  **step-back** question. Each first passes a cheap **scope gate** — does it
  plausibly relate to anything actually ingested, per a one-sentence
  auto-generated **source catalog**? If clearly not, it skips straight to web
  search. Otherwise it's retrieved through a hybrid of vector similarity,
  BM25 keyword search, **self-query** metadata filtering (informed by that
  same source catalog), and **parent-document** retrieval, then reranked by a
  local cross-encoder — all per (sub-)question, not once against the
  combined question (see [Query analysis](#query-analysis) for why that
  ...
  **graded for relevance**; only if it falls short does the system fall back
  ...
  [Self-correcting generation](#self-correcting-generation).
...
- **Multi-format ingestion** — PDF, DOCX, TXT, Markdown, and web pages
  (unstructured text, chunked with overlap-aware recursive splitting), plus
  **CSV, Excel, and JSON** (structured data, converted row-by-row into
  retrievable text — see [Structured data](#structured-data)).
...
- **Parent-document retrieval** — small chunks get embedded and matched (so
  ...
  small chunks doesn't get returned as an incomplete fragment (see
...
- **Self-query retrieval, grounded in what sources actually cover** — an LLM
  ...
  explicitly — informed by a real, auto-generated description of what each
  source covers, not just its filename (see
...
- **A source catalog, generated once and reused** — a one-sentence,
  ...
  self-query's filtering and the scope gate below (see
...
- **A scope gate in front of retrieval** — before running the full local
  ...
  straight to web search instead of paying for vector/BM25/self-query
  retrieval and relevance grading on a question local docs were never going
  to answer (see [Scope gate](#scope-gate)).
- **Web search, used correctively** — a live Tavily search backs the system
  ...
  answer it (see [Corrective RAG](#corrective-rag-c-rag)).
...
- **Corrective RAG (C-RAG)** — retrieved documents are graded for relevance
  ...
  first retrieval pass returned (see [Corrective RAG](#corrective-rag-c-rag)).
- **Self-correcting generation (Self-RAG/RRR)** — the generated *answer* is
  ...
  question triggers a question rewrite and a fresh retrieval pass — the
  generation-quality-gated correction that Corrective RAG's retrieval-side
  grading structurally can't catch (see
  [Self-correcting generation](#self-correcting-generation)).
- **Text-to-SQL** — for structured data, a second query path lets the LLM
  ...
  the most sections?") get correct answers instead of no answer at all (see
...
- **Logical routing** — a single chat interface, not three separate tools:
  ...
  path accordingly (see [Routing](#routing)).
- **Query analysis (decomposition + step-back)** — a compound question
  ("what's the drop deadline, and what's the mission statement?") gets split
  into atomic sub-questions, each retrieved and reranked on its own merits,
  instead of one half silently winning the single retrieval pass; a broader
  "step-back" question is also generated and retrieved alongside the original,
  for background context a narrowly-worded question might otherwise miss
  (see [Query analysis](#query-analysis)).
...
- **Source citations, actually grounded in metadata** — each retrieved passage
  is tagged with its real filename/URL *before* the LLM ever sees it
  (`chain.py`'s `document_prompt`), so citations name the passage's real
  source instead of guessing from surface text — see
  [Notes on design choices](#notes-on-design-choices) for the bug this fixed.
  The API also returns the list of sources retrieved.
- **Evaluation harness** — [RAGAS](https://github.com/explodinggradients/ragas)
  ...
  not just vibes.
- **Two interfaces** — a CLI for local use and a FastAPI backend with session
  management, ready to sit behind any frontend.
...
```
   ...
   Query analysis: decomposition                   ▼
    ...
    step-back (one broader question)   connection + SELECT-only
                    │                  validation, THEN executes
                    ▼                              │
   For each (sub-/step-back) question:              ▼
   Scope gate: plausibly related        Plain-language answer
   to ANY known source (source           from the real result
   catalog)? If clearly not →
   skip straight to web search.
   Otherwise:
     Vector (MMR) + BM25 +
     self-query (auto metadata,
     catalog-informed filters) +
...
rankings — see [Text-to-SQL](#text-to-sql)), build the queryable database too:
...
path can actually answer it (see [Routing](#routing)):
...
step, no npm — mounted by FastAPI itself
...
dishonesty flagged above for the old Streamlit "Stop" button, just moved to
a different control. `/query/stream` sends the real thing instead:
newline-delimited JSON, one line per actual stage transition
...
build filterable attributes from.
...
entirely rather than run and simply come up empty.
...
Two independent checks, run in `pipeline.py`'s `ChatSession._ask_vectorstore`
...
Fixed to `range(MAX_HALLUCINATION_RETRIES + 1)`, the same "+1" shape already
used for the outer rewrite loop — every regenerated answer now gets
re-verified, not blindly trusted.
...
4. **The scope gate runs once, against the original question** —
   `ScopeGatedRetriever` now wraps `DecomposingRetriever` instead of the
   other way around, so `check_scope` is checked before decomposition
   rather than once per sub-question after it. Scoping a compound question
   as a whole is safe here (unlike reranking/correction, which do need to
   run per sub-question — see the `build_hybrid_retriever` docstring): the
   gate only needs "does ANY of this relate to something we have," which a
   single pass answers correctly. For a clearly out-of-scope question this
   also means decomposition never runs at all, since there's nothing local
   left to decompose for. Verified directly, not assumed: profiling "What
   is the capital of France?" after the change shows exactly one
   `check_scope` call and zero `decompose`/`step_back` calls (11.7s total,
   vs. call counts that used to scale with sub-question count).
5. **Self-query construction is now conditional** (`self_query.py`'s
   `ConditionalSelfQueryRetriever`). Building a metadata filter is itself an
   LLM call, run on every (sub-)question in the ensemble regardless of
   whether the question has anything to do with metadata filtering. It's
   now gated on a free, non-LLM keyword-overlap check against known source
   filenames and catalog descriptions — self-query only runs when the
   question shares real vocabulary with a known source, same conservative
   fail-open default as the scope gate (any overlap at all still runs it).
...
src/
  ...
  rag/
    ...
    self_query.py      auto-extracted metadata filters (SelfQueryRetriever)
    ...
    query_analysis.py  query decomposition + step-back prompting (DecomposingRetriever)
    ...
    pipeline.py        ingest + ChatSession orchestration (routes + dispatches)
...
- **Answer style: fuller and more natural, still strictly grounded**. The
  original `ANSWER_PROMPT` ("precise research assistant... ONLY the
  context") produced technically-correct but terse answers — often a single
  sentence lifted close to verbatim from the source, even when the retrieved
  context had more relevant detail sitting right there unused. Rewritten to
  explicitly ask for full sentences and to include supporting detail already
  present in the context (conditions, related specifics) rather than
  trimming to the bare minimum — while keeping every other constraint
  (cite the real `[source: ...]` tag, never invent one, say so if the
  context doesn't cover it) unchanged. Verified on real data, not assumed:
  the same mission-statement question that used to return one quoted
  sentence now returns two paragraphs of actual explanation, every claim
  still cited to the same source. The self-correction hallucination check
  (`self_correction.py`) still grades the fuller answer the same way, so
  the added length can't come from ungrounded claims without getting
